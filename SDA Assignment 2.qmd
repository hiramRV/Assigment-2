...
We create three multiple regression models: one with all the variables, one without any log transformations, and one using the log of the starting price and the area. We start by including all interactions between the covariates, and use stepwise backwards selection to remove abundant interactions and variables. Then we visualize the residuals, Q-Q graph and density plot of each one: 
