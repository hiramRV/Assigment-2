...
We create three multiple regression models: a "baseline" model without any transformations or interactions, one with interactions but without transformations, and one using the log of the starting price and the area and incorporating the interactions among variables. We start by including all interactions between the covariates, and use stepwise backwards selection to remove abundant interactions and variables.
...
(Pablos part to motivate selection of model)

The best multiple regression model is as follows:
$$
...
```
Prediction ($\hat{y}$) for every new value using model 3.
```{r}
pred3[,1]
```

Prediction Intervals with 90% confidence for each new value.
```{r}
pred3[,2:3]
```
