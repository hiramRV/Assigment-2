...
We can assume that the distribution of $\hat{p}$ is approximately normal (since sample estimators for samples of suffiecient size are normally distributed around the real value). For the distribution, we use mean 0.5 ($H_{0}$) and standard deviation 0.019.  Formally, the distribution is as follows:
...
data %>%
  ...
  theme_classic()
```
The distribution as shown by the histogram is is left tailed and not clearly normally distributed. However, because the sample size is greater than 30 we can approximate the distribution of $\bar{x}$ with a normal distribution using the central limit theorem.
