...
Linear regressions enable us to make predictions for a variable given a different set of variables. In this case, we can predict the Starting Price of the housing units as a function of Region, Type, Balcony, Rooms and Area as well as interactions and transformations amongst them. While this is very powerful, linear regression models make several assumptions that must be met if we want to use it. First, that the relationship between the independent variables and the dependent variable is linear. Second, that the error terms are independent. Third, that the error terms have a constant standard deviation. Fourth and finally, that the error terms are normaly distributed. While the latter three assumptions can only be checked after fitting the model (as they primarily relate to the error terms or the differences between the predicted and real values), the first can be checked ex-ante. Therefore, before creating and comparing different multiple regression models, we must first check that our data fits with the linearity assumption, and conduct appropriate transformations as needed.

We can do this rapidly by visualizing our data, efficiently checking the relationship between our variables as well as evaluating if any transformation may be necessary. We begin with the relationship between Rooms and Starting Price.
...
The first graph shows an approximately linear relationship between Rooms and Starting Price, and there is no clear improvement by transforming. As such, we considered it unnecessary to use any transformation. 
...
While the plot without any transformations shows a fairly linear relationship between the starting price and area of housing units, it also displays more heteroskedasticity for larger housing units. Taking the log of both starting price and area renders a linear and homoskedastic plot, and could be a good option for further analysis and model creation.

We did not repeat this procedure for Balcony, Type and Region as these variables are categorical and non-ordinal.
...
A third model without rooms. 

```{r}
m3 = lm(STARTING_PRICE~REGION+TYPE+AREA+BALCONY, data=data) #No rooms
```
...
```{r}
pred1 <- predict(m1,test,interval = "prediction", level = 0.90)
pred2 <-exp(predict(m2,test,interval = "prediction", level = 0.90))
pred3 <-predict(m3,test,interval = "prediction", level = 0.90)
