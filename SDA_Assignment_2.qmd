...
(Pablo writes about why we select the third model (Adj R-squared, OLS assumptions of residuals))

The best model is written out below. The reference group is apartments in region Northeast without a balcony. This model includes all variables that are significant on a 10% level, and interactions are marked by a colon symbol.
$$
\begin{align}
